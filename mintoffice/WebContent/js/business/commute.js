						...
						customButtons : {
							arrive : {
								...
								click : function() {
									...
									$('#checkMsg')
											.text(
													'Do you want to record your arrival time?');
							...
							gohome : {
								...
								click : function() {
									...
									$('#checkMsg')
											.text(
													'Do you want to finish your work today?');
							...
							goOut : {
								...
								click : function() {
									...
									$('#checkMsg')
											.text(
													'Do you want to record your outside office work?');
							...
							comeBack : {
								...
								click : function() {
									...
									$('#checkMsg')
											.text(
													'Do you want to end your outside work recording?');
						...
						eventRender : function(event, element) {
							element.attr("com_late", event.categoryname),
									...
									element.find('.fc-title').css(
											'font-weight', 'bold').css('color',
											'#202020');
							element.find('.fc-content').append(
									'<div>' + event.sTime + '~' + event.eTime
											+ '</div>').css('color', '#404040');
							/*
							 * element.find('.fc-content').append('<div>Start&nbsp;:&nbsp;' +
							 * event.sTime+'</div>').css('color','#404040');
							 * element.find('.fc-content').append('<div>End&nbsp;:&nbsp;' +
							 * event.eTime+'</div>').css('color','#404040');
							 */
							/*
							 * element.find('.fc-content').append('<input
							 * type="text" name="modify_no"
							 * style="display:none;" value='+event.id+'>');
							 */
							/*
							 * element.find('.fc-title').append("<br/>" +
							 * event.place); element.find('.fc-title').append("<br/>" +
							 * event.content);
							 */
							element.find('.fc-content').on(
									'click',
									function() {
										if (event.kind == 0) {
											$('#modifyPS').modal();
					...
					$.ajax({
						...
						success : function(data) {
							...
							if (data == 1) { // success
								openMsgModal();
								$('#workMsg')
										.text('Arrival has been recorded.')
										.css('color', '#43A479');
			...
			function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data == 1) { // success
							openMsgModal();
							$('#workMsg').text('Go-home has been recorded.')
									.css('color', '#43A479');
						} else if (data.indexOf('Opps') != -1) {
							openMsgModal();
			...
			function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data == 1) { // success
							openMsgModal();
							$('#workMsg').text('Go-Out has been recorded.')
									.css('color', '#43A479');
						} else if (data.indexOf('Opps') != -1) {
							openMsgModal();
							$('#workMsg').text(data).css('color', '#FF4500');
			...
			function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data == 1) { // success
							openMsgModal();
							$('#workMsg').text('Comeback has been recorded.')
									.css('color', '#43A479');
						} else if (data.indexOf('Opps') != -1) {
							openMsgModal();
							$('#workMsg').text(data).css('color', '#FF4500');
			...
			function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data == 1) { // success
							openMsgModal();
							$('#workMsg').text('Illness has been recorded.')
									.css('color', '#43A479');
						} else if (data.indexOf('Opps') != -1) {
							openMsgModal();
							$('#workMsg').text(data).css('color', '#FF4500');
